...
async def place_order(
    ...
    """
    # 1. Allergen Gate: verify passenger allergies if a profile exists
    profile = db.scalar(
        select(PassengerProfile).where(PassengerProfile.passenger_id == current_user.id)
    )
    if profile:
        allergy_flags = profile.allergy_flags or {}
        # Preload all ordered meals to check allergens
        meal_ids = [item.meal_id for item in payload.items]
        meals = db.scalars(select(MealItem).where(MealItem.id.in_(meal_ids))).all()
