  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!allowed.includes(role)) {
        setError(`This isn't an ${EXPECTED} account. Please use the correct login page for your role.`);
        setLoading(false);
        return;
      }
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-6">
      ...
      <div className="relative flex w-full max-w-md flex-col rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface)] p-9 shadow-[var(--shadow-xl)] backdrop-blur-2xl transition-all duration-300 hover:-translate-y-1 hover:border-[var(--color-border-light)] hover:shadow-[0_20px_50px_rgba(99,102,241,0.15)]">
        ...
        <p className="mb-8 text-center text-sm text-[var(--color-text-secondary)]">
          Analytics, AI models, and flight operations
        </p>
        ...
        <form onSubmit={handleSubmit} className="mt-2 flex flex-col gap-[18px] text-left" noValidate>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Input
              ...
              placeholder="admin@sv.airmeal.demo"
            />
          </div>
